...
async def search_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Yahan badlaav kiya gaya hai: Check karein ki message private chat se hai ya nahi
    if update.message.chat.type != 'private':
        return  # Agar message group ya channel se hai to kuch na karein
...
async def handle_back_callbacks(query, user_id: int, data: str):
    ...
    if destination == "main":
        welcome_text = formatter.format_welcome_message(
            query.effective_user.first_name or "there"
        )
